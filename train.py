...
def train_mlp_epoch(epoch, args, rnn, output, data_loader,
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        # sort input
        y_len,sort_index = torch.sort(y_len_unsorted,0,descending=True) # 이게 BFS? 내림차순
        y_len = y_len.numpy().tolist()
        x = torch.index_select(x_unsorted,0,sort_index)
        y = torch.index_select(y_unsorted,0,sort_index)
        x = Variable(x).cuda()
...
def get_rnn_loss(args, rnn, output, data_loader):
#    rnn.train()
#    output.train()
    loss_sum = 0
    for batch_idx, data in enumerate(data_loader):
        ...
        ## MSE
#        loss_temp = y_pred - output_y
#        loss = loss_temp**2
#        pytorch_tensor = loss.cpu()
#        np_tensor = pytorch_tensor.detach().numpy()
#        loss = tf.convert_to_tensor(np_tensor, dtype= tf.float64)        
...
def test_rnn_epoch(EOS, max_num_node, M, epoch, args, rnn, output, test_batch_size=16):
    rnn.hidden = rnn.init_hidden(test_batch_size)
    rnn.eval()
    output.eval()
    ...
    #max_num_node = int(args.max_num_node)
    #max_num_node = 9
    y_pred_long = Variable(torch.zeros(test_batch_size, max_num_node, M)).cuda() # discrete prediction
    x_step = Variable(torch.ones(test_batch_size,1, M)).cuda()
    ...
    for i in range(EOS):
        ...
        x_step = Variable(torch.zeros(test_batch_size,1,M)).cuda()
