...
class Graph_sequence_sampler_pytorch(torch.utils.data.Dataset):
    def __init__(self, G_list, max_num_node=None, max_prev_node=None, iteration=200):
        self.adj_all = []
        self.len_all = []
        for G in G_list:
    ...
    def __getitem__(self, idx):
        ...
        #x_idx = np.random.permutation(adj_copy.shape[0]) #20200706 comment out
        #adj_copy = adj_copy[np.ix_(x_idx, x_idx)] #20200706 comment out
        adj_copy_matrix = np.asmatrix(adj_copy)
        G = nx.from_numpy_matrix(adj_copy_matrix)
        # then do bfs in the permuted G
        #start_idx = 1
#        start_idx = np.random.randint(adj_copy.shape[0]) #20200706 comment out
#        x_idx = np.array(bfs_seq(G, start_idx))  #20200706 comment out
#        adj_copy = adj_copy[np.ix_(x_idx, x_idx)] #20200706 comment out
    ...
    def calc_max_prev_node(self, iter=2000,topk=10):
        max_prev_node = []
        for i in range(iter):
            #if i % (iter / 5) == 0:
